...
new Vue({
  ...
  watch: {
    currentIndex (val) {
      if (val > this.questionNumAdd(this.questionInfo.question_typenum, 1)
       && val < this.questionNumAdd(this.questionInfo.question_typenum, 2)) {
  ...
  methods: {
    questionNumAdd (arr, num) { // 返回某个数组的前几项之和
      var numadd = 0
      for (var i = 0; i < num; i++) {
        numadd += arr[i]
      }
